...
function updateCart() {
  ...
  cart.forEach((item, idx) => {
    ...
    div.className = "flex justify-between items-center text-sm border-b pb-2 gap-2";
    ...
    div.innerHTML = `
      ...
      <div class="flex items-center gap-1">
        <button class="cart-minus px-2 py-1 bg-gray-200 rounded" data-idx="${idx}">−</button>
        <button class="cart-plus px-2 py-1 bg-gray-200 rounded" data-idx="${idx}">+</button>
        <button class="cart-remove px-2 py-1 bg-red-500 text-white rounded" data-idx="${idx}"><i class="fas fa-trash"></i></button>
      </div>
...
// Cart undo logic
const undoBtnCart = document.getElementById("undoBtnCart");

let lastRemovedItem = null;
let lastRemovedIndex = null;
let undoTimeout = null;

// Update your cartItems event listener:
...
// Show Undo button in cart header
function showUndoCart() {
  undoBtnCart.classList.remove("hidden");
  if (undoTimeout) clearTimeout(undoTimeout);
  undoTimeout = setTimeout(() => {
    undoBtnCart.classList.add("hidden");
    lastRemovedItem = null;
    lastRemovedIndex = null;
  }, 5000);
}

undoBtnCart.addEventListener("click", () => {
...
checkoutBtn.addEventListener("click", () => {
  ...
  cart.forEach(item => {
    const itemTotal = item.qty * item.price;
    total += itemTotal;
    receipt += `• ${item.name} x${item.qty} - ₱${item.price.toFixed(2)} each = ₱${itemTotal.toFixed(2)}\n`;
  });
  receipt += "\n-----------------------------\n";
